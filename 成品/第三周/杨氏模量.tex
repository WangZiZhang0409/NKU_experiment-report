...
\begin{document}
	% 制作封面
	\thispagestyle{empty} % 将第一页的页面样式设置为empty,此样式不会显示页眉和页脚
	\vspace*{0.5cm}	% 空0.5cm
	\begin{figure}[h] % h选项指代here，表示按照出现的顺序给出图片
		\centering % 居中
		\includegraphics[width=0.7\linewidth]{logo} % 你大南开的logo啦，图片我一同放在了文件夹中
	\end{figure}
	% 标题
	\vspace*{0.1cm}
	\begin{center}
	...
	\end{center}
	% 给出个人信息
	\begin{table}[h] % 同figure的选项
		\centering	
		\begin{Large} % 设置字号环境为 large
			\begin{tabular}{p{3cm} p{5cm}<{\centering}}% 这里的<\centering 使用了array宏包，表明第二列的属性为centering
				姓\qquad 名: & xxx \\
				\hline
				学\qquad 院: & xxx \\
				\hline
